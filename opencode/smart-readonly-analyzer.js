...
const inlineSecretPatterns = [
  ...
  // Note: no trailing \b because = is not a word character, use lookahead instead
  /\b[A-Za-z0-9_-]{32,}={1,2}(?=\s|"|'|$)/g,  // base64 with padding (ends in = or ==)
  /\b(?=[A-Za-z0-9+/]*[+/])[A-Za-z0-9+/]{32,}(?=\s|"|'|$)/g,  // 32+ chars with at least one + or /
];
...
const analyzeWithAI = async (client, redactedCommand) => {
  try {
...
export const SmartReadOnlyAnalyzer = async ({ client }) => {
  ...
  return {
    "permission.ask": async (args) => {
      ...
      const analysis = await analyzeWithAI(client, redacted);
...
// Attach internals for testing (not a separate export to avoid plugin loader issues)
SmartReadOnlyAnalyzer._internals = {
  sensitivePathPatterns,
  shellOperatorPatterns,
  readOnlyPatterns,
  dangerousPatterns,
  bareSensitiveFilenames,
  inlineSecretPatterns,
  isReadOnly,
  parseCommandChain,
